...
class HarmonicCompression(BaseCompression):
    ...
    def n_compressed(self) -> int:
        """Size of compressed space (n_modes)."""
        return self.n_modes
    ...
    def _allocate_buffers(self) -> None:
        ...
        """
        # Buffer for K matrix in SMW: (n_modes, n_modes)
        self._K_buffer = np.empty(
            (self.n_modes, self.n_modes), dtype=np.float64, order="F"
        )
        # Buffer for M @ K_inv @ M product: (n_modes, n_modes)
