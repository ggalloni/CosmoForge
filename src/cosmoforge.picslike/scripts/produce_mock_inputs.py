...
names = Par.parameters.keys()
amplitude_values = np.linspace(
    Par.parameters["amplitude"][0],
    Par.parameters["amplitude"][1],
    Par.parameters["amplitude"][2],
)
tilt_values = np.linspace(
    Par.parameters["tilt"][0], Par.parameters["tilt"][1], Par.parameters["tilt"][2]
)

os.makedirs(f"{outpath}/templates", exist_ok=True)
...
print(f"Generated grid of D_l spectra for parameters: {', '.join(names)}")
...
fiducial = np.loadtxt(f"{outpath}/templates/dls_amplitude{1.0:.6g}_tilt{0.0:.6g}.txt")

new_clfid[:, 1:] = fiducial[:, 1:] / todl[:, None]
...
for i in tqdm(range(nsims), desc="Producing simulations".center(30)):
    ...
    sim_tqu = hp.alm2map(alm, nside=nside, lmax=Par.lmax, pol=True) + noise
    ...
    sim_teb = hp.alm2map(alm, nside=nside, lmax=Par.lmax, pol=False) + noise
...
print("All simulations generated successfully with shape:", sim_tqu.shape)
