...
- **`n_components` (basis layer)** — A **field** count, not a component count in the *Sky geometry* sense below: `n_components = len(self._theta_tuple)` counts pointing sets, and `_n_pix_per_component` doubles for spin-2, so one QU "component" block holds both Q and U. The name is a misnomer, documented rather than renamed in 1.3.0 (ADR 0020); a rename is breaking on a public basis attribute and would need an ADR-0018 shim. Consequence: everything the basis layer indexes per "component" (`_pix_offsets`, `_V_blocks`, field-block detection) is field-granular, so a Q-U mixing filter never leaves its block and only a T-P mixing one crosses a block boundary.

## Filters (ADR 0020)

- **Filter** — A linear pixel-space operator `F` the data has already been through (scan-polynomial removal, harmonic deprojection, template regression, hits weighting), declared to the estimator as `pixel_filter=` on `Fisher`, `Spectra` and `PICSLike`. Realised as a frozen `cosmocore.filters.Filter` record holding the thin SVD `F = U Σ Wᵀ` truncated at the rank threshold. Never the *act* of filtering: CosmoForge does not filter maps, it analyses maps that were filtered.
- **Restriction** — How a filter enters: the analysis is carried out inside `range(F)`, in `r = rank` dimensions, rather than by conjugating `n × n` objects and regularising the null space. Raw inputs restrict as `Σ Wᵀ d`, matrices as `Σ (Wᵀ X W) Σ`. `cosmocore.basics.restrict` / `restrict_data` are the primitives.
- **Conjugation** — The dense alternative, `C → F C Fᵀ` plus a ridge on the null space. Not a library path; kept as the acceptance-test oracle, and then only on the **truncated** operator `F_trunc = U_k Σ_k W_kᵀ`, because an oracle built on the full `F` is a different estimator.
- **Rank threshold** — The user-exposed bias/variance knob deciding how much of `F` survives. Two knobs at two layers: `template_epsilon` on the `cosmocore.filters` constructors (relative cut on the template stack) and `range_epsilon` **or** `range_rank` on `Filter` (mutually exclusive, cut on `F`'s own singular values). Applied to `F`, never to `C`. There is deliberately no default. `range_epsilon` clips at **`MIN_RANGE_EPSILON`** (`sqrt(eps)`, about 1.5e-8) with a warning, because internal coordinates carry `Σ` and the restricted covariance therefore carries `Σ²`: conditioning goes as `range_epsilon⁻²`.
- **Projector vs graded** — A projector has every kept singular value equal to 1, so `U` *is* `W` (the same object) and `Σ = I`; deprojection and purification are projectors. A graded filter has a non-trivial `Σ`: a transfer function, a soft high-pass, hits weighting. The distinction is stored on the record, not inferred, because raw and pre-filtered inputs must land in identical coordinates for a projector.
- **Pre-filtered input** — An input array that already carries the filter, declared with `maps_prefiltered` / `noise_prefiltered` and restricted with `Uᵀ` instead of `Σ Wᵀ`. `noise_prefiltered` means "the covariance of the noise in the maps as handed", never "the filter whitened my noise". Identical to the raw convention for a projector; different by `Σ²`, hence a different noise model, for a graded filter.
- **No-op control** — Hits weighting. Diagonal and positive, hence invertible, and a conjugated estimator is invariant under invertible maps, so restriction must leave the answer unchanged. It is the control that shows the machinery is transparent when it should be, not a filter anyone needs to apply.
...
- **Bins** — Bandpower binning class in cosmocore. `Bins.fromdeltal(lmin, lmax, delta_ell)`. Knows bin geometry only: edges, widths, shape weights. It does not know where a bandpower sits (see *effective multipole*). Adapted from xQML (must cite Vanneste+ 2018).
...
- **Bandpower shape function** (ADR 0019) — The weights `w_ℓ` in the binned derivative. They *declare* what the spectrum is assumed to do inside a bin; the estimator returns the amplitude of that shape. `w_ℓ = 1` declares C_ℓ flat in the bin (estimator returns `C_b`); `w_ℓ = 2π/(ℓ(ℓ+1))` declares D_ℓ flat in the bin (returns `D_b`). Selected by `output_convention`, which is therefore an *estimator* setting, not a presentation one: `Cl` and `Dl` measure different observables, and `D_output ≠ scalar × C_output`.
- **Effective multipole** (ADR 0019) — Where a bandpower sits on the ℓ axis: the centroid `Σ_ℓ W̃[b,ℓ]·ℓ` of the estimator's realised window (ADR 0005), normalised in the estimated quantity's own space (`W̃ = W ⊙ w`). *Computed*, not declared — QML applies inverse-variance weighting automatically, so the centroid depends on noise and mask coupling and is knowable only from the per-ℓ Fisher. Differs per spectrum and between `Cl` and `Dl`.
- **Bin midpoint** (`Bins.lmid`) — `(lmin + lmax)/2` for a bin. A cheap label, available with no Fisher run. **Not** the effective multipole, and not a valid C→D conversion point.
...
- **Input injection** (ADR-0017) — Every pipeline input is loadable through two adapters: a file path in `InputParams` or an in-memory object passed as a constructor kwarg (the injected object wins). Fixed kwarg vocabulary: `mask`, `noise_cov1`/`noise_cov2`, `maps1`/`maps2` (Spectra, PICSLike), `cls_data`, `fiducial_cls`, `beam` — each named after the in-memory object it becomes, not the path it shadows. Extended by ADR-0020 with `pixel_filter` (an in-memory input with no path to shadow; 1.3.0 ships no YAML expression of a filter) and the two declarations `maps_prefiltered` / `noise_prefiltered`, which describe an input rather than being one.
