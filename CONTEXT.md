...
- **Bandpower shape function** (ADR 0019) — The weights `w_ℓ` in the binned derivative. They *declare* what the spectrum is assumed to do inside a bin, and the estimator returns the amplitude of that shape. `w_ℓ = 1` declares C_ℓ flat in the bin (the estimator returns `C_b`); `w_ℓ = 2π/(ℓ(ℓ+1))` declares D_ℓ flat in the bin (it returns `D_b`). Selected by `output_convention`, which is therefore an *estimator* setting, not a presentation one: `Cl` and `Dl` mode measure different observables and `D_output ≠ scalar × C_output`. There is no post-hoc C→D conversion; a scalar applied to an already-collapsed bin cannot be exact.
- **Effective multipole** — Where a bandpower actually sits on the ℓ axis: the centroid `Σ_ℓ W̃[b,ℓ]·ℓ` of the estimator's realised window (ADR 0005), normalised in the estimated quantity's own space (`W̃ = W ⊙ w`). It is *computed*, not declared — QML applies inverse-variance weighting automatically, so the centroid depends on noise and mask coupling and is knowable only from the per-ℓ Fisher. Differs per spectrum (TT/EE/BB have different noise) and between `Cl` and `Dl` mode. Lives on `Fisher`; `Spectra` delegates.
- **Bin midpoint** (`Bins.lmid`) — `(lmin + lmax)/2` for a bin. A cheap label, available with no Fisher run. **Not** the effective multipole and not a valid C→D conversion point; it was formerly called `lbin` and doubled as both, which is the conflation ADR 0019 removes.
...
- **Input injection** (ADR-0017) — Every pipeline input is loadable through two adapters: a file path in `InputParams` or an in-memory object passed as a constructor kwarg (the injected object wins). Fixed kwarg vocabulary: `mask`, `noise_cov1`/`noise_cov2`, `maps1`/`maps2` (Spectra, PICSLike), `cls_data`, `fiducial_cls`, `beam` — each named after the in-memory object it becomes, not the path it shadows.
